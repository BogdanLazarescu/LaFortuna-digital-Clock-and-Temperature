...
void display_char(char c) {
    //CHANGE font dimension here
    uint16_t x, y,i,j;
    uint16_t fontH = display.fontH, fontW=display.fontW;
	//Set the reference to your new added font here:
	char *fontC;
	if(fontH==7 && fontW==8) fontC= font7x8;else
	if(fontH==12 && fontW==8) fontC= font12x8;else
	if(fontH==18 && fontW==24) fontC= font18x24;else
	if(fontH==40 && fontW==32) fontC= font40x32;
	else return;
    PGM_P fdata; 
    uint8_t mask;
    uint16_t sc=display.x, ec=display.x + fontW, sp=display.y, ep=display.y + fontH;

    /*   New line starts a new line, or if the end of the
         display has been reached, clears the display.
    */
    if (c == '\n') { 
        display.x=0; display.y+=fontH;
        if (display.y >= display.height) { clear_screen(); }
        return;
    }

    if (c < ' ' || c > '~') return;
    fdata = (c - ' ')*fontH*(fontW/8) + fontC;
    if(fontH ==40) fdata = (c - '0')*fontH*(fontW/8) + fontC;
    write_cmd(PAGE_ADDRESS_SET);
    write_data16(sp);
    write_data16(ep);
	...
	for(i=0; i <fontH;i++){
		for(j=0; j<(fontW/8); j++){
		bits[i][j] = pgm_read_byte(fdata++);
		}
	}
    for(x=sc,i=0; x<ec; x++,i++) {
        write_cmd(COLUMN_ADDRESS_SET);
        write_data16(x);
        write_data16(x);
        write_cmd(MEMORY_WRITE);
       // bits = pgm_read_byte(fdata++);
        for(y=0; y<fontH; y++){
		mask=0x01;
       		write_data16((bits[y][i/8] & (mask<<=(7-(i%8)))) ? display.foreground : display.background);
	}
    }
    write_cmd(COLUMN_ADDRESS_SET);
    write_data16(x);
    ...
    display.x += fontW;
   if (display.x >= display.width) { display.x=0; display.y+=fontH+1; }
}
...
void display_font(uint16_t H, uint16_t W) {
	display.fontH = H;
	display.fontW = W;
